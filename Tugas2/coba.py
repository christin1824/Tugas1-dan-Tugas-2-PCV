    ...
    def draw_animated_avatar(self, blank, pose_keypoints, hand_keypoints, faces, threshold=0.5):
        ...
        if 11 in points and 12 in points and 23 in points and 24 in points:
            ...
            try:
                ...
                new_w = max(1, int((x2 - x1) * TOP_SCALE_X))
                new_h = max(1, int((y2 - y1) * TOP_SCALE_Y))
        ...
        if 0 in points:
            ...
            cv2.circle(blank, (left_eye_pos[0] + 7, right_eye_pos[1] + 5), 3, (255, 255, 255), -1)
            # Outline mata
...
class TkinterTrackerApp:
    def __init__(self, tracker):
        ...
        # --- Variabel Deteksi Warna HSV (Tugas 2) ---
        
        # Rentang HSV untuk mendeteksi warna BIRU (H: 100-120)
        self.lower_blue = np.array([100, 100, 100])
        self.upper_blue = np.array([120, 255, 255]) 

        # Rentang HSV untuk mendeteksi warna HIJAU (H: 35-75)
        self.lower_green = np.array([35, 100, 100])
        self.upper_green = np.array([75, 255, 255])
        
        # Rentang HSV untuk mendeteksi warna MERAH (Merah terbagi menjadi dua rentang)
        # Bagian 1: Hue Rendah (Dekat 0)
        self.lower_red_1 = np.array([0, 100, 100])
        self.upper_red_1 = np.array([10, 255, 255])
        # Bagian 2: Hue Tinggi (Dekat 179)
        self.lower_red_2 = np.array([160, 100, 100])
        self.upper_red_2 = np.array([179, 255, 255])

        # Kernel untuk operasi morfologi (5x5)
        self.morph_kernel = np.ones((5, 5), np.uint8)
        
        # Status Deteksi Aksi
        self.detected_color = None # Bisa: 'Red', 'Green', 'Blue', atau None
        self.MIN_CONTOUR_AREA = 500 # Luas minimum kontur untuk memicu aksi

        # Setup window
        self.root = tk.Tk()
        self.root.title("MediaPipe Tracker - Animated Avatar with HSV Detection")
        ...
        tk.Label(main, text="Webcam Input (HSV Mask Overlay)", **label_style).grid(row=0, column=0, pady=6)
        tk.Label(main, text="Animated Avatar (HSV Color Action)", **label_style).grid(row=0, column=1, pady=6)
        ...
        # Binding Tugas 1 DIHAPUS

        self.delay = 15
        self.update()
        self.root.mainloop()
    
    # Fungsi set_filter TIDAK DIPERLUKAN karena Tugas 1 dihapus
    ...
    def update(self):
        ...
        if ret:
            ...
            # --- START: Logika Deteksi Warna HSV (Tugas 2) ---
            
            # 1. Konversi ke HSV
            hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

            # 2. Thresholding Warna (R, G, B)
            mask_b = cv2.inRange(hsv, self.lower_blue, self.upper_blue)
            mask_g = cv2.inRange(hsv, self.lower_green, self.upper_green)
            mask_r1 = cv2.inRange(hsv, self.lower_red_1, self.upper_red_1)
            mask_r2 = cv2.inRange(hsv, self.lower_red_2, self.upper_red_2)
            mask_r = cv2.bitwise_or(mask_r1, mask_r2)
            
            mask_combined = cv2.bitwise_or(mask_r, cv2.bitwise_or(mask_g, mask_b))

            # 3. Pembersihan Mask (Morfologi)
            mask_cleaned = cv2.morphologyEx(mask_combined, cv2.MORPH_OPEN, self.morph_kernel)
            mask_cleaned = cv2.morphologyEx(mask_cleaned, cv2.MORPH_CLOSE, self.morph_kernel)
            ...
            for contour in contours:
                ...
                if area > self.MIN_CONTOUR_AREA:
                    # Tentukan warna kontur yang terdeteksi
                    M = cv2.moments(contour)
                    if M["m00"] != 0:
                        cx = int(M["m10"] / M["m00"])
                        cy = int(M["m01"] / M["m00"])
                        
                        # Cek mask mana yang aktif di titik tengah kontur yang besar
                        if mask_r[cy, cx] > 0:
            ...
            # Draw tracking overlay & HSV Mask overlay (Canvas 1)
            processed_frame = frame.copy()
            # Overlay mask di canvas 1 untuk visualisasi deteksi
            mask_rgb = cv2.cvtColor(mask_cleaned, cv2.COLOR_GRAY2BGR)
            processed_frame = cv2.addWeighted(processed_frame, 1.0, mask_rgb, 0.5, 0)
            ...
            if current_bg is not None:
                ...
                # Pemicu Aksi (Tugas 2 Aksi): Ganti warna gradient berdasarkan deteksi
                if self.detected_color == 'Red':
                    # Pemicu Merah: Background menjadi Biru (kontras)
                    bg_color_start = [255, 0, 0]   
                    bg_color_end = [200, 0, 0]
                    action_text = "Status: OBJEK MERAH TERDETEKSI!"
                elif self.detected_color == 'Green':
                    # Pemicu Hijau: Background menjadi Merah (kontras)
                    bg_color_start = [0, 0, 255]   
                    bg_color_end = [0, 0, 200]
                    action_text = "Status: OBJEK HIJAU TERDETEKSI!"
                elif self.detected_color == 'Blue':
                    # Pemicu Biru: Background menjadi Hijau (kontras)
                    bg_color_start = [0, 255, 0]   
                    bg_color_end = [0, 200, 0]
                    action_text = "Status: OBJEK BIRU TERDETEKSI!"
                else: 
                    # Normal: Dark Blue/Gradient
                    bg_color_start = [10, 10, 20]
                    bg_color_end = [30, 30, 40]
                    action_text = "Status: Normal"

                # Generate Gradient
                for i in range(self.height):
                    ratio = i / self.height
                    b = int(bg_color_start[0] * (1 - ratio) + bg_color_end[0] * ratio)
                    g = int(bg_color_start[1] * (1 - ratio) + bg_color_end[1] * ratio)
                    r = int(bg_color_start[2] * (1 - ratio) + bg_color_end[2] * ratio)
                    bg_raw[i, :] = [b, g, r]
            
            # Masukkan background ke blank canvas 
            blank = bg_raw.copy()
            
            # Tambahkan Teks status deteksi (Tugas 2 Aksi)
            text_color = (255, 255, 255) # Putih default
            if self.detected_color:
                 # Menggunakan warna yang kontras dengan background yang berubah (BGR)
                 text_color = (0, 255, 255) if self.detected_color == 'Red' else (255, 0, 0)
            
            cv2.putText(blank, action_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, text_color, 2, cv2.LINE_AA)

            # 4. Gambar karakter animasi di atas background
            self.tracker.draw_animated_avatar(blank, pose_keypoints, hand_keypoints, faces)

            # Convert BGR->RGB and to ImageTk
            img1 = cv2.cvtColor(processed_frame, cv2.COLOR_BGR2RGB) 
